...
trainingAccuracies = zeros(10, 1);
testingAccuracies = zeros(10, 1);
...
for target_user = 1:10
    ...
    for u = 1:10
        ...
        if u == target_user
            ...
            numTargetTest = round(0.3 * numTargetSamples);
            targetTestData = userData(targetIndices(numTargetTrain+1:end), :);
            ...
            numImposterSelect = round(0.3 * numImposterSamples);
            ...
            imposterTestData = userData(imposterIndices(numImposterSelect+1:end), :);
    ...
    % Store results
    trainingAccuracies(target_user) = trainingAccuracy;
    testingAccuracies(target_user) = testingAccuracy;
...
resultsTable = table(users', trainingAccuracies, testingAccuracies, ...
    'VariableNames', {'User', 'TrainingAccuracy', 'TestingAccuracy'});
disp(resultsTable);
